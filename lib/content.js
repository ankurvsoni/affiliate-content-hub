...
function mdToHtml(md) {
  ...
  while (i < lines.length) {
    ...
    if (t.startsWith('# ')) {
      ...
      if (currentSection.includes('top picks')) {
        const m = rawItem.match(/amazon\.com\/dp\/([A-Z0-9]{10})/i) || rawItem.match(/\/([A-Z0-9]{10})\/\?tag=/i);
        if (m) {
          const asin = m[1].toUpperCase();
          const thumb = `https://images-na.ssl-images-amazon.com/images/P/${asin}.01._SX140_.jpg`;
          out.push(`<li><div class="pickMedia"><img class="pickThumb" src="${thumb}" alt="Product image" loading="lazy" onload="this.parentElement.classList.add('loaded')" onerror="this.parentElement.classList.add('failed');this.remove()"/></div>${itemHtml}</li>`);
        } else {
